functions{
  real root_kernel_lpdf(real s, real lambda, real alpha){
    /*if(s<mu){
      return log(alpha)- log(lambda) + Z;
    }else{
      return pareto_type_2_lpdf(s|mu,lambda,alpha) + Z;
    }*/
    return pareto_type_2_lpdf(s|0.0,lambda,alpha);
  }
  real root_kernel_lcdf(real s, real lambda, real alpha){
    /*if(s<mu){
      return log(s) + log(alpha) - log(lambda) + Z;
    }else{
      return log_sum_exp(log(mu) + log(alpha) - log(lambda),pareto_type_2_lcdf(s|mu,lambda,alpha)) + Z ;
    }*/
    return pareto_type_2_lcdf(s|0.0,lambda,alpha);
  }
  real kernel_lpdf(real s, real gamma,real kappa){
      return exponential_lpdf(s|gamma);
      //return pareto_type_2_lpdf(s|0.0,gamma,sigma);
  ...
  real kernel_lcdf(real s, real gamma,real kappa){
    return exponential_lcdf(s|gamma);
    //return pareto_type_2_lcdf(s|0.0,gamma,sigma);
...
transformed parameters{
  ...
  vector[S] kappa[2];
  real gamma[2] = inv_logit(gamma_raw);
  {
    for(k in 1:2){
      for(s in 1:S){
        kappa[k,s] = exp(l_M[k,s]-l_l_2/gamma[k]);
      }
    }
